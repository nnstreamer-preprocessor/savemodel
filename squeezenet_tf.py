...
def main():
    ...
    classes = []
    with open('labels.txt', 'r') as classes_file:
    ...
    if options.fool is not None:
        ...
        with g.as_default(), tf.Session(config=config) as sess:
            ...
            tf.train.write_graph(sess.graph_def, './', 'squeeze1.pb', as_text=True)
        ...
        with g.as_default(), tf.Session(config=config) as sess:
            ...
            tf.train.write_graph(sess.graph_def, './', 'squeeze2.pb', as_text=True)
